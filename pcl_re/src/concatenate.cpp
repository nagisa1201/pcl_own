...
{
    pcl::PointCloud<pcl::PointXYZ> cloud_a, cloud_b, cloud_c;//储存三个点坐标的点云
    pcl::PointCloud<pcl::Normal> n_cloud_b;//存储法线信息
    pcl::PointCloud<pcl::PointNormal> p_n_cloud_c;//存储点和法线信息

    cloud_a.width  = 5;
    cloud_a.height = cloud_b.height = n_cloud_b.height = 1;
    cloud_a.resize (cloud_a.width * cloud_a.height);//根据对应的宽度和高度调整点云大小
    ...
    {
        pcl::concatenateFields (cloud_a, n_cloud_b, p_n_cloud_c);//最后一个传入变量是输出变量
        std::cerr << "Cloud C: " << std::endl;
